...
class GetListeningStatsUseCase extends AbstractUseCase
{
    ...
    protected function doExecute(mixed ...$args): array
    {
        ...
        if ($query->statsType === 'album_info') {
            ...
            try {
                $raw    = $this->lastFmService->getAlbumInfo($query->artist, $query->album, $lastfmUsername ?: null);
                $parsed = $this->lastFmService->parseAlbumInfo($raw);
        ...
        $raw = match ($query->statsType) {
            'user_info'          => $this->lastFmService->getUserInfo($lastfmUsername),
            'top_albums'         => $this->lastFmService->getTopAlbums($lastfmUsername, $query->period, $query->limit),
            'top_artists'        => $this->lastFmService->getTopArtists($lastfmUsername, $query->period, $query->limit),
            'top_tracks'         => $this->lastFmService->getTopTracks($lastfmUsername, $query->period, $query->limit),
            'recent_tracks'      => $this->lastFmService->getRecentTracks($lastfmUsername, $query->limit),
            'loved_tracks'       => $this->lastFmService->getLovedTracks($lastfmUsername, $query->limit),
            'weekly_album_chart' => $this->lastFmService->getWeeklyAlbumChart($lastfmUsername),
            'weekly_artist_chart'=> $this->lastFmService->getWeeklyArtistChart($lastfmUsername),
