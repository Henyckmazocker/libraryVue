<template>
  <div class="edition-notes">
    ...
    <div v-else-if="sortedNotes.length > 0" class="notes-list">
      ...
      >
        <div class="note-header">
          <div class="note-meta">
            <i :class="['pi', editionNotes.getNoteTypeIcon(note.noteType || note.note_type)]"></i>
            <span class="note-type">{{ editionNotes.getNoteTypeLabel(note.noteType || note.note_type) }}</span>
            <span class="note-page">Página {{ note.pageNumber || note.page_number }}</span>
    ...
    <div v-else class="empty-state">
      ...
      <p class="empty-hint">Agrega notas para recordar tus pensamientos mientras lees</p>
    </div>
...
import { useEditionNotes } from '@/composables/useEditionNotes'
...
// Composables
const editionNotes = useEditionNotes()
...
const sortedNotes = computed(() => {
  return [...editionNotes.notes.value].sort((a, b) => {
...
const loading = computed(() => editionNotes.loading.value)

// Methods
async function loadNotes() {
  if (!props.userEditionId) {
    Logger.warn('No userEditionId provided, cannot load notes')
    return
  }
  
  Logger.info('Loading notes for edition', { userEditionId: props.userEditionId })
  const result = await editionNotes.getNotes(props.userEditionId)
...
function editNote(note) {
  ...
  noteForm.value = {
    pageNumber: note.pageNumber || note.page_number,
...
async function deleteNote(note) {
  const result = await editionNotes.deleteNote(note.id)
  if (result.success) {
    if (notifications) {
...
async function saveNote() {
  ...
  try {
    ...
    if (editingNote.value) {
      ...
      result = await editionNotes.updateNote(editingNote.value.id, noteForm.value)
    } else {
      // Add new note
      result = await editionNotes.addNote(
        props.userEditionId,
        noteForm.value.pageNumber,
...
// Watch for user edition ID changes
watch(() => props.userEditionId, (newId) => {
  if (newId) {
    loadNotes()
  }
}, { immediate: true })
