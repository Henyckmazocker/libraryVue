<template>
  <div class="game-notes">
    ...
    <div v-else-if="sortedNotes.length > 0" class="notes-list">
      ...
      >
        <div class="note-header">
          <div class="note-meta">
            <i :class="['pi', gameNotes.getNoteTypeIcon(note.noteType || note.note_type)]"></i>
            <span class="note-type">{{ gameNotes.getNoteTypeLabel(note.noteType || note.note_type) }}</span>
    ...
    <div v-else class="empty-state">
      ...
      <p class="empty-hint">Agrega notas para recordar tus experiencias con este juego</p>
    </div>
...
const sortedNotes = computed(() => {
  return [...gameNotes.notes.value].sort((a, b) => {
...
const loading = computed(() => gameNotes.loading.value)

// Methods
async function loadNotes() {
  if (!props.gameId) {
    Logger.warn('No gameId provided, cannot load notes')
    return
  }
  
  Logger.info('Loading notes for game', { gameId: props.gameId })
  const result = await gameNotes.getNotes(props.gameId)
...
async function deleteNote(note) {
  const result = await gameNotes.deleteNote(note.id, props.gameId)
  if (result.success) {
    if (notifications) {
...
async function saveNote() {
  ...
  try {
    ...
    if (editingNote.value) {
      ...
      result = await gameNotes.updateNote(
      ...
      // Add new note
      result = await gameNotes.addNote(
        props.gameId,
        noteForm.value.noteText,
        noteForm.value.noteType,
        noteForm.value.isPrivate
